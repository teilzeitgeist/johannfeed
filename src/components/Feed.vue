<template>
    <div v-if="events.length > 0" class="event-container">
        ...
        <Flicking v-if="chunkedEvents.length > 0"
            :options="{ defaultIndex: 0, duration: 400, align: 'prev', circular: true, horizontal: true, adaptive: true, resizeOnContentsReady: true }"
            :plugins="plugins" @moveStart="onMove">
            <div v-for="(chunk, i) in chunkedEvents" class="panels" :key="i">
                <div class="grid-container">
                    <event v-for="(event, index) in chunk" :key="event.guid" :event="event" :data-index="index"
                        class="event panel" :colorClass="`event--slide-${(i % 5) + 1}`" />
    ...
    <div v-else class="empty">
        <div class="text"><span v-if="!isError">Wird geladen</span><span v-else>Leider ist beim Laden ein Fehler
                aufgetreten. Wir kümmern uns bestimmt bereits darum ;-)</span></div>
